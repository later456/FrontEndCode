	...
	moveTo: function(a, b, c) {
		...
		this._addToQueue(function(d) {
			...
			var f = $.proxy(function(e, f) {
				...
				f === clippy.Animator.States.WAITING && this._el.animate({
					top: b,
					left: a
				},
				c, $.proxy(function() {
...
clippy.Animator.prototype = {
	...
	showAnimation: function(a, b) {
		return this._exiting = !1,
		this.hasAnimation(a) ? (this._currentAnimation = this._data.animations[a], this.currentAnimationName = a, this._started || (this._step(), this._started = !0), this._currentFrameIndex = 0, this._currentFrame = undefined, this._endCallback = b, !0) : !1
	},
	_draw: function() {
		var a = [];
	...
	_step: function() {
		...
		this._endCallback && b && this._atLastFrame() && (this._currentAnimation.useExitBranching && !this._exiting ? this._endCallback(this.currentAnimationName, clippy.Animator.States.WAITING) : this._endCallback(this.currentAnimationName, clippy.Animator.States.EXITED))
	},
	pause: function() {
...
clippy.Balloon.prototype = {
	...
	_position: function(a) {
		...
		case "top-left":
			g = b.left + d - f,
			h = b.top - e - this._BALLOON_MARGIN;
			break;
		case "top-right":
			g = b.left,
			h = b.top - e - this._BALLOON_MARGIN;
		...
		case "bottom-right":
			g = b.left,
			h = b.top + c + this._BALLOON_MARGIN;
			break;
		case "bottom-left":
			g = b.left + d - f,
			h = b.top + c + this._BALLOON_MARGIN
	...
	pause: function() {
		window.clearTimeout(this._loop),
		this._hiding && (window.clearTimeout(this._hiding), this._hiding = null)
	},
	resume: function() {
...
clippy.BASE_PATH = "http://lab.mkblog.cn/clippy/agents/",
...
clippy.load._loadAgent = function(a, b) {
	...
	var d = b + "/agent.js";
	return clippy.load._loadScript(d),
	c.promise()
},
...
clippy.load._getAgentDfd = function(a) {
	...
	return b || (b = clippy.load._data[a] = $.Deferred()),
	b
},
...
clippy.soundsReady = function(a, b) {
	var c = clippy.load._sounds[a];
	c || (c = clippy.load._sounds[a] = $.Deferred()),
	c.resolve(b)
},
clippy.Queue = function(a) {
	this._queue = [],
	this._onEmptyCallback = a
},
...
window.PR_SHOULD_USE_CONTINUATION = !0; (function() {
	function L(a) {
		...
		function y(a) {
			...
			for (i = c = 0; c < b; ++c) j = f[c],
			j === "(" ? (++i, d[i] === void 0 && (f[c] = "(?:")) : "\\" === j.charAt(0) && (j = +j.substring(1)) && j <= i && (f[c] = "\\" + d[i]);
			for (i = c = 0; c < b; ++c)"^" === f[c] && "^" !== f[c + 1] && (f[c] = "");
			if (a.ignoreCase && s) for (c = 0; c < b; ++c) j = f[c],
			a = j.charAt(0),
			j.length >= 2 && a === "[" ? f[c] = h(j) : a !== "\\" && (f[c] = j.replace(/[A-Za-z]/g,
			function(a) {
				a = a.charCodeAt(0);
	...
	function u(a) {
		...
		a.regexLiterals && e.push(["lang-regex", /^(?:^^\.?|[!+-]|!=|!==|#|%|%=|&|&&|&&=|&=|\(|\*|\*=|\+=|,|-=|->|\/|\/=|:|::|;|<|<<|<<=|<=|=|==|===|>|>=|>>|>>=|>>>|>>>=|[?@[^]|\^=|\^\^|\^\^=|{|\||\|=|\|\||\|\|=|~|break|case|continue|delete|do|else|finally|instanceof|return|throw|try|typeof)\s*(\/(?=[^*/])( ? :[ ^ /[\\]|\\[\S\s]|\[(?:[^\\\]]|\\[\S\s])*(?:]|$))+\/)/]); (h = a.types) && e.push(["typ", h]);
		a = ("" + a.keywords).replace(/^ | $/g, "");
		a.length && e.push(["kwd", RegExp("^(?:" + a.replace(/[\s,]+/g, "|") + ")\\b"), q]);
		m.push(["pln", /^\s+/, q, " \r\n\t\xa0"]);
		e.push(["lit", /^@[$_a-z][\w$@]*/i, q], ["typ", /^(?:[@_]?[A-Z]+[a-z][\w$@]*|\w+_t\b)/, q], ["pln", /^[$_a-z][\w$@]*/i, q], ["lit", /^(?:0x[\da-f]+|(?:\d(?:_\d+)*\d*(?:\.\d*)?|\.\d\+)(?:e[+-]?\d+)?)[a-z]*/i, q, "0123456789"], ["pln", /^\\[\S\s]?/, q], ["pun", /^.[^\s\w"-$'./@\\`] * /,q]);return x(m,e)}function D(a,m){function e(a){switch(a.nodeType){case 1:if(k.test(a.className))break;if("BR"===a.nodeName)h(a),
	...
	["pun", /^[/ < ->] + /],["lang-js",/ ^ on\w + \s *= \s * "([^"] + )"/i],["lang - js ",/^on\w+\s*=\s*'([^']+)'/i],["lang - js ",/^on\w+\s*=\s*([^\s"'>]+)/i],["lang-css",/^style\s*=\s*"([^"]+)"/i],["lang-css",/^style\s*=\s*' ([ ^ ']+)' / i], ["lang-css", /^style\s*=\s*([^\s"'>]+)/i]]),
	["in.tag"]);
	k(x([], [["atv", /^[\S\s]+/]]), ["uq.val"]);
	k(u({
		keywords: F,
	...
	window.prettyPrint = function(a) {
			...
			for (var e = window.PR_SHOULD_USE_CONTINUATION ? l.now() + 250 : Infinity; p < h.length && l.now() < e; p++) {
				...
				if (k.indexOf("prettyprint") >= 0) {
					...
					if (b = !k) {
						b = n;
						for (var o = void 0,
						c = b.firstChild; c; c = c.nextSibling) var i = c.nodeType,
		...
		for (var e = [document.getElementsByTagName("pre"), document.getElementsByTagName("code"), document.getElementsByTagName("xmp")], h = [], k = 0; k < e.length; ++k) for (var t = 0,
		s = e[k].length; t < s; ++t) h.push(e[k][t]);
		var e = q,
		l = Date;
		l.now || (l = {
			now: function() {
...
$(function() {
	...
	// twitter counter
	$.ajax('//cdn.api.twitter.com/1/urls/count.json', {
		data: {
			url: url
		},
		dataType: 'jsonp',
