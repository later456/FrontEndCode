function style_html(h, i, f) {
    ...
    d = new function () {
        ...
        this.get_content = function () {
            for (var a = "", c = [], b = !1; this.input.charAt(this.pos) !== "<";) {
                ...
                if (this.Utils.in_array(a, this.Utils.whitespace)) c.length && (b = !0), this.line_char_count--;
                else {
                    if (b) {
                        ...
                        } else c.push(" "), this.line_char_count++;
                        b = !1
                    }
                    c.push(a)
                }
        ...
        this.retrieve_tag = function (a) {
            if (this.tags[a + "count"]) {
                ...
                if (c) this.indent_level = this.tags[a + this.tags[a + "count"]], this.tags.parent = this.tags[c + "parent"];
        ...
        this.get_tag = function () {
            ...
            do {
                ...
                if (this.Utils.in_array(a, this.Utils.whitespace)) b = !0, this.line_char_count--;
                    ...
                    if (a === "'" || a === '"')
                        if (!c[1] || c[1] !== "!") a += this.get_unformatted(a), b = !0;
                    a === "=" && (b = !1);
            ...
            a.charAt(a.length - 2) === "/" || this.Utils.in_array(b, this.Utils.single_token) ? this.tag_type = "SINGLE" : b === "script" ? (this.record_tag(b), this.tag_type = "SCRIPT") : b === "style" ? (this.record_tag(b), this.tag_type = "STYLE") : b.charAt(0) === "!" ? b.indexOf("[if") != -1 ? (a.indexOf("!IE") != -1 && (a = this.get_unformatted("--\>", a), c.push(a)), this.tag_type = "START") : b.indexOf("[endif") != -1 ? (this.tag_type =
                "END", this.unindent()) : (a = b.indexOf("[cdata[") != -1 ? this.get_unformatted("]]\>", a) : this.get_unformatted("--\>", a), c.push(a), this.tag_type = "SINGLE") : (b.charAt(0) === "/" ? (this.retrieve_tag(b.substring(1)), this.tag_type = "END") : (this.record_tag(b), this.tag_type = "START"), this.Utils.in_array(b, this.Utils.extra_liners) && this.print_newline(!0, this.output));
            return c.join("")
        };
        ...
        this.get_token = function () {
            ...
            if (this.current_mode === "CONTENT") return a =
                this.get_content(), typeof a !== "string" ? a : [a, "TK_CONTENT"];
            if (this.current_mode === "TAG") return a = this.get_tag(), typeof a !== "string" ? a : [a, "TK_TAG_" + this.tag_type]
        };
        this.printer = function (a, c, b, d) {
            ...
            this.print_newline = function (a, c) {
                ...
                if (c && c.length) {
                    if (!a)
                        for (; this.Utils.in_array(c[c.length -
                                1], this.Utils.whitespace);) c.pop();
                    c.push("\n");
    ...
    for (d.printer(h, f, i);;) {
        ...
        case "TK_TAG_SINGLE":
            ...
            break;
        case "TK_CONTENT":
            d.token_text !== "" && (d.print_newline(!1, d.output), d.print_token(d.token_text)), d.current_mode = "TAG"
        }
...
function js_beautify(h, i, f, d) {
    ...
    function B() {
        ...
        if (a > 1)
            for (var d = 0; d < 2; d++) c(d === 0);
        a = a === 1;
        if (q(b, y)) {
            if (e < j.length)
                for (; q(j.charAt(e), y);)
                    if (b += j.charAt(e), e += 1, e === j.length) break;
            if (e !== j.length && b.match(/^[0-9]+[Ee]$/) && j.charAt(e) === "-") return e += 1, a = B(e), b += "-" + a[0], [b, "TK_WORD"];
        ...
        if (b === "'" || b === '"' || b === "/" && (g === "TK_WORD" && p === "return" || g === "TK_START_EXPR" || g ===
                "TK_END_BLOCK" || g === "TK_OPERATOR" || g === "TK_EOF" || g === "TK_END_COMMAND")) {
            a = b;
            d = !1;
            b = "";
            if (e < j.length)
                for (; d || j.charAt(e) !== a;)
                    if (b += j.charAt(e), d = d ? !1 : j.charAt(e) === "\\", e += 1, e >= j.length) break;
            e += 1;
            g === "TK_END_COMMAND" && c();
    ...
    for (f = !1;;) {
        ...
        case "TK_START_EXPR":
            ...
            v.push(o);
            o = "EXPRESSION";
            g === "TK_END_EXPR" || g === "TK_START_EXPR" ||
                (g !== "TK_WORD" && g !== "TK_OPERATOR" ? b() : q(h, i) && h !== "function" && b());
            m();
            break;
        case "TK_END_EXPR":
            m();
        ...
        case "TK_OPERATOR":
            ...
            if (k === ",") {
                s ? w ? (m(), c(), w = !1) : (m(), b()) :
                    g === "TK_END_BLOCK" ? (m(), c()) : o === "BLOCK" ? (m(), c()) : (m(), b());
                break
...
onDecodeClick = function () {
    try {
        ...
        a.html('0');
        $("#tme").html('0');
        for (d.attr('disabled',"true"); i.test(f.val()) && c < 20;) addinfo("正在解码第" + (c + 1) + "次..."), f.val(eval(f.val().replace(i, "(function(){ return (") + "})()")), c++;
        i.test(f.val()) ? (addinfo("解码失败，已经经过" +
            c + "次尝试", 2), $("#errmsg").html("非常遗憾，解码失败。")) : (c > 0 ? addinfo("解码完成，共解码" + c + "次", 3) : addinfo("原文似乎没有编码，不需要解码", 3), addinfo("正在以" + ($("#tabsize option:selected").val() == 1 ? "制表符" : $("#tabsize option:selected").val() + "空格") + "缩进格式化源码..."), do_js_beautify(), addinfo("格式化完成"));
        a.html(c);
    } catch (b) {
        addinfo("解码失败，过程发生错误: " + b.message, 2), $("#errmsg").html("发生错误：" + b.message);
    }
...
onEncodeClick = function () {
    try {
        ...
        addinfo("编码失败，过程发生错误: " + i.message, 2), $("#errmsg").html("发生错误：" + i.message);
    }
